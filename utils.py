...
def load_file(file_path):
    """
    Loads the pickele file into a dataframe.
    ...
    df = pd.merge(left=tmp["dataset"]["X"], right=tmp["dataset"]["Y"],how="left",left_index=True, right_index=True)
...
class Client:
    def __init__(self, client_id, dataset):
        ...
        self.X_train = pd.DataFrame(scaler_x.fit_transform(X_train.values),index=X_train.index, columns=X_train.columns)
        self.X_test = pd.DataFrame(scaler_x.transform(X_test.values),index=X_test.index, columns=X_test.columns)
    ...
    def set_model(self, model, parameters_dict = None):
       self.model = model
       #self.curr_param = parameters_dict
