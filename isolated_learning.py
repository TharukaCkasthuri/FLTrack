...
for client in clients:
    ...
    for epoch in tqdm(range(epochs)):
        client_model, train_loss = client.train(client_model, loss_fn, optimizer, epoch)
    ...
    model_path = (
        checkpt_path + "batch" + str(batch_size) + "_client_" + str(client_id) + ".pth"
    )
    ...
    loss_df.to_csv(
        "losses/" + "batch" + str(batch_size) + "_client_" + str(client_id) + ".csv",
        index=False,
    )
writer.flush()
writer.close()
